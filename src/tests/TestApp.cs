...
namespace CSE.WebValidate.Tests.Unit
{
    public class TestApp
    {
        ...
        public void EnvironmentVariableTest()
        {
            ...
            System.Environment.SetEnvironmentVariable(EnvKeys.TelemetryKey, "012345678901234567890123456789012345");
            ...
            Assert.Equal(13, parse.CommandResult.Children.Count);

            // override the files env var
            parse = root.Parse("-f file1 file2");
            Assert.Equal(0, parse.Errors.Count);
            Assert.Equal(13, parse.CommandResult.Children.Count);
            // TODO - make sure there are two files
        ...
        {
            using Config cfg = BuildConfig("https://www.microsoft.com");
            cfg.Files.Add("msft.json");
