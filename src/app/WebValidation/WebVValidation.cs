...
namespace WebValidation
{
    public partial class WebV : IDisposable
    {
        ...
        public static string ValidateStatusCode(Request r, HttpResponseMessage resp)
        {
            ...
            if ((int)resp.StatusCode != r.Validation.StatusCode)
            {
                res += string.Format(CultureInfo.InvariantCulture, $"\tStatusCode: {(int)resp.StatusCode} Expected: {r.Validation.StatusCode}\n");
            }
        ...
        public static string ValidateExactMatch(Request r, string body)
        {
            ...
            {
                // compare values
                if (r.Validation.ExactMatch != null && body != r.Validation.ExactMatch)
                {
                    res += string.Format(CultureInfo.InvariantCulture, $"\tExactMatch: Actual : {body.PadRight(40).Substring(0, 40).Trim()} : Expected: {r.Validation.ExactMatch.PadRight(40).Substring(0, 40).Trim()}\n");
                }
            }
        ...
        public static string ValidateJsonArray(Request r, string body)
        {
            ...
            if (r.Validation.JsonArray != null)
            {
                try
                {
                    ...
                    // validate min count
                    if (r.Validation.JsonArray.MinCount != null && r.Validation.JsonArray.MinCount > resList.Count)
                    {
